...
class Agent:
    ...
    多个 Agent 不代表多个不同模型：它们通常调用同一模型，但各自拥有不同的
    ...
    def __init__(self, instruction, role, examplers=None, model_info='gpt-4o-mini', img_path=None):
        ...
        elif self.model_info in OPENAI_COMPAT_MODELS:
            self.client = openai_compatible_client(self.model_info)
            # 对话历史。后续 chat() 会持续向此列表追加消息。
            self.messages = [
                {"role": "system", "content": instruction},#system：角色设定，告诉模型“你是谁、要遵守什么原则”。
            ]
            # 可以提供示例：把题目、答案和理由预先放进对话上下文。
    ...
    def chat(self, message, img_path=None, chat_mode=True):
        ...
        if self.model_info in OPENAI_COMPAT_MODELS:
...
class Group:
    ...
    def __init__(self, goal, members, question, examplers=None, model_info='gpt-4o-mini'):
        ...
        self.members = []
        self.model_info = model_info
        # 这里将每一位成员真正实例化为一个 Agent。
        for member_info in members:
            _agent = Agent('You are a {} who {}.'.format(member_info['role'], member_info['expertise_description'].lower()), role=member_info['role'], model_info=self.model_info)
    ...
    def interact(self, comm_type, message=None, img_path=None):
        ...
        if comm_type == 'internal':
            ...
            try:
                ...
                delivery = assist_members[0].chat(delivery_prompt)#如果没有leader
...
def create_question(sample, dataset):
    ...
    if dataset == 'medqa':
        ...
        for k, v in sample['options'].items():
            options.append("({}) {}".format(k, v)) #把例如 ：A 左心房（ k:A v:左心房）
...
def process_basic_query(question, examplers, model, args):
    ...
    # 示例在 Agent 初始化时加入 messages，最终提问时模型即可模仿其答题格式。
...
def process_intermediate_query(question, examplers, model, args):
    ...
    """recruited是一段文本，表示要招聘哪些岗位,例如 1. Cardiologist - Focuses on heart disease. - Hierarchy: Independent
    2. Pulmonologist - Focuses on lung disease. - Hierarchy: Cardiologist > Pulmonologist"""
    ...
    # 树只用于打印展示；实际通信记录保存在 interaction_log 中。
    ...
    for agent in agents_data:
        #try / except 的作用是：如果招募员 AI 没有严格按规定格式输出，文本拆不开，就跳过这一行，避免程序立即崩溃。
        try:
            agent_role = agent[0].split('-')[0].split('.')[1].strip().lower()
            description = agent[0].split('-')[1].strip().lower()
        except:
...
#准备开始讨论
    ...
    # 预先建好“轮次 → 回合 → 发言者 → 接收者”的通信矩阵，方便最后打印交流关系。
    ...
    # Moderator 不是简单计数器，而是一个新 Agent，读取各专家的最终文本后重新生成结论。
...
def process_advanced_query(question, model, args):
    ...
    # 招募结果是文本，必须严格符合 Group/Member 格式，后续 parse_group_info 才能把它变成团队对象。
